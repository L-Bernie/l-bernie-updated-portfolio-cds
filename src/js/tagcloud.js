(function() {
  ...
  function Y(aI, aH) {
    ...
    return (
      (j &&
        j.getComputedStyle &&
        j.getComputedStyle(aI, null).getPropertyValue(aH)) ||
  ...
  function u(j, aI, aH) {
    ...
    if (aI) {
      ...
      if ((aJ = Y(j, "font-size"))) {
        i =
          (aJ.indexOf("px") > -1 && aJ.replace("px", "") * 1) ||
          (aJ.indexOf("pt") > -1 && aJ.replace("pt", "") * 1.25) ||
          aJ * 3.3;
      }
    }
    return i;
  ...
  function f(i) {
    return i.target && ai(i.target.id)
      ? i.target.id
      : i.srcElement.parentNode.id;
  }
  function S(aJ, aK) {
    var aI,
      aH,
      i = parseInt(Y(aK, "width")) / aK.width,
    ...
    if (ai(aJ.offsetX)) {
      ...
      if (aJ.pageX) {
        aI = { x: aJ.pageX - aH.x, y: aJ.pageY - aH.y };
      }
    }
    if (aI && i && j) {
      aI.x /= i;
  ...
  x.DrawColour = function(aI, aL, aJ, aM, aH, i, aN, j, aK) {
    ...
    return this[aN.image ? "DrawColourImage" : "DrawColourText"](
      aI,
      aL,
      aJ,
      aM,
  ...
  x.DrawPulsate = function(aL, i, j, aI) {
    var aJ = G() - this.ts,
      ...
      aK =
        aH.pulsateTo +
        (1 - aH.pulsateTo) *
          (0.5 + w(2 * Math.PI * aJ / (1000 * aH.pulsateTime)) / 2);
  ...
  c.Init = function(j) {
    ...
    this.SetShadowColour = i.shadowAlpha
  ...
  c.SetDraw = function(i) {
    this.Draw = this.fimage
      ? i.ie > 7 ? this.DrawImageIE : this.DrawImage
      : this.DrawText;
      ...
      if (aP.outlineMethod == "colour") {
        ...
        if (aP.outlineMethod == "size") {
          aN = t(
            this.text,
            this.textFont,
            this.textHeight + aP.outlineIncrease
          );
          i = aN.max.y + aN.min.y;
          aJ =
            aQ * (this.textHeight + aP.outlineIncrease) + "px " + this.textFont;
          aM.font = aJ;
          aH = this.MeasureText(aM);
          aK = new g(
            this.text,
          ...
          if (this.image) {
            aK.SetImage(
              this.image,
              this.iw + aP.outlineIncrease,
              this.ih + aP.outlineIncrease,
              aP.imagePosition,
              aP.imagePadding,
              aP.imageAlign,
              aP.imageVAlign,
            ...
            if ("size" == j) {
              if (aI.weightSizeMin > 0 && aI.weightSizeMax > aI.weightSizeMin) {
                this.textHeight =
                  aI.weightSize *
                  (aI.weightSizeMin +
                    (aI.weightSizeMax - aI.weightSizeMin) * i);
              } else {
  ...
  function y(aN, j, aI) {
    ...
    this.animTiming =
      typeof y[this.animTiming] == "function" ? y[this.animTiming] : y.Smooth;
  ...
  Q.Weight = function(aN) {
    var aJ = aN.length,
      ...
      aK = [],
      j,
      aI = this.weightFrom ? this.weightFrom.split(/[, ]/) : [null],
      aM = aI.length;
    for (aH = 0; aH < aJ; ++aH) {
      aK[aH] = [];
      for (aO = 0; aO < aM; ++aO) {
  ...
  Q.Load = function() {
    var aR = this.GetTags(),
      ...
      aO,
      aK = [],
      aN = { sphere: q, vcylinder: am, hcylinder: au, vring: d, hring: n };
    if (aR.length) {
      aK.length = aR.length;
    ...
    if (aM && this.fixedAnim.active) {
      ...
      for (aV = 0; aV < aU; ++aV) {
        aY =
          this.mx >= 0 &&
          this.my >= 0 &&
          this.taglist[aV].CheckActive(aX, aP, aO);
